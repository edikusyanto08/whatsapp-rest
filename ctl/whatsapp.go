...
import (
	...
	"github.com/xtwoend/whatsapp-rest/hlp"
	"github.com/xtwoend/whatsapp-rest/hlp/auth"
	"github.com/xtwoend/whatsapp-rest/hlp/libs"
...
func WhatsAppSendDocument(w http.ResponseWriter, r *http.Request) {
	...
	mpFileStream, mpFileHeader, err := r.FormFile("file")
	if err != nil {
		router.ResponseBadRequest(w, err.Error())
		return
	}
	...
	mpFileType := mpFileHeader.Header.Get("Content-Type")
	mpFileName := mpFileHeader.Filename
	...
	err = libs.WAMessageDocument(jid, reqBody.MSISDN, mpFileStream, mpFileType, mpFileName, reqBody.Delay)
	if err != nil {
		router.ResponseInternalError(w, err.Error())
		return
	}
