...
func WAInit(jid string, timeout int) error {
	if wac[jid] == nil {
		...
		conn.SetClientName("Whatsapp Nusapay", "Go WhatsApp")
		wac[jid] = conn
	}
...
func WAMessageDocument(jid string, jidDest string, msgFileStream multipart.File, msgFileType string, msgFileName string, msgDelay int) error {
	if wac[jid] != nil {
		jidPrefix := "@s.whatsapp.net"
		if len(strings.SplitN(jidDest, "-", 2)) == 2 {
			jidPrefix = "@g.us"
